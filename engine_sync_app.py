...
class SyncManager:
    ...
    def motor_sincronizacao(self, ui_strings, progress_callback, db_path):
        ...
        try:
            ...
            for idx, caminho_completo in enumerate(arquivos_totais):
                ...
                if row_existente:
                    ...
                    if self.config.get("importar_hotcue") and _HOTCUE_DISPONIVEL and caminho_completo.lower().endswith('.mp3'):
                        updated_hotcue_for_track = self._importar_hotcue_track(
                            cursor=cursor, 
                        ...
                        if updated_hotcue_for_track:
                            hotcue_manipulated_count += 1
                ...
                try:
                    ...
                    if self.config.get("importar_hotcue") and _HOTCUE_DISPONIVEL and caminho_completo.lower().endswith('.mp3'):
                        updated_hotcue_for_track = self._importar_hotcue_track(
                            ...
                            progress_callback=progress_callback)
                        if updated_hotcue_for_track: hotcue_manipulated_count += 1
                except Exception as e:
            ...
            self.log(log_paths, f"  Hotcues manipulados    : {hotcue_manipulated_count}")
            ...
            resumo_final = [
                ...
                f"Músicas com Hotcues Manipulados: {hotcue_manipulated_count}",
                f"Playlists na Árvore           : {len(tracks_por_playlist)}",
                f"Total de Referências          : {total_entidades}",
                "="*60
            ]
            
            return novas_musicas, apagadas_musicas, hotcue_manipulated_count, "\n".join(report_lines + resumo_final)
        finally:
            conn.close()