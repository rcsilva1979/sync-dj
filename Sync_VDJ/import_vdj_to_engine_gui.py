...
class PlaylistContentWindow(ctk.CTkToplevel):
    def __init__(self, master, title, xml_path):
        ...
        lbl = ctk.CTkLabel(self, text=f"Lista de Músicas: {title}", font=ctk.CTkFont(size=14, weight="bold"), text_color="#00E5A3")
        lbl.pack(pady=10)
        ...
        try:
            ...
            if not songs:
                self.textbox.insert("end", "Nenhuma música encontrada nesta playlist.")
            else:
            ...
            self.textbox.insert("end", f"Erro ao ler XML: {e}")
...
class ImportVDJToEngineWindow(ctk.CTkToplevel):
    def __init__(self, master, txt_strings):
        ...
        self.title(self.txt.get("vdj_export_btn", "Exportar Playlist do VDJ para o Engine"))
    ...
    def build_ui(self):
        ...
        lbl_title = ctk.CTkLabel(self, text=self.txt.get("vdj_export_btn", "Exportar Playlist do VDJ para o Engine"), 
        ...
        ctk.CTkLabel(vdj_frame, text="Selecionar Playlist do VirtualDJ (.vdjfolder):", font=ctk.CTkFont(weight="bold")).pack(anchor="w")
        ...
        if self.vdj_options:
            self.selected_vdj_playlist.set(self.vdj_options[0])

        # Info sobre bancos detectados
        dbs_info = " | ".join([os.path.splitdrive(d)[0] for d in self.found_databases])
        lbl_dbs = ctk.CTkLabel(self, text=f"Bancos Engine Detectados: {dbs_info}", font=ctk.CTkFont(size=11), text_color="#00E5A3")
        lbl_dbs.pack(pady=(5, 0))

        # Status e Progresso
        self.lbl_status = ctk.CTkLabel(self, text="Selecione a playlist para começar.", font=ctk.CTkFont(size=12), text_color="#AAAAAA", wraplength=450)
        ...
        # Botão de Execução
        self.btn_run = ctk.CTkButton(self, text="Iniciar Importação para o Engine", font=ctk.CTkFont(size=14, weight="bold"),
                                     fg_color="#00E5A3", text_color="#000000", hover_color="#00b37e", height=45, width=350, 
    ...
    def show_playlist_contents(self):
        ...
        if not xml_path or not os.path.exists(xml_path):
            messagebox.showerror("Erro", "Selecione uma playlist válida para visualizar seu conteúdo.")
            return

        PlaylistContentWindow(self, display_name, xml_path)
    ...
    def run_import(self):
        ...
        if not xml_path or not os.path.exists(xml_path):
            messagebox.showerror("Erro", "Selecione um arquivo XML do VirtualDJ válido.")
            return
        ...
        try:
            self.update_status(f"Lendo playlist VDJ...", "#00E5A3")
            tree = ET.parse(xml_path)
            root = tree.getroot()
            ...
            if total_tracks == 0:
                messagebox.showwarning("Aviso", "Nenhuma faixa encontrada no arquivo XML.")
                return

            processed_count = 0
            for drive, tracks in tracks_by_drive.items():
                db_path = dbs_by_drive[drive]
                self.update_status(f"Importando {len(tracks)} faixas para o banco do disco {drive}...", "#00E5A3")
                ...
                for vtrack in tracks:
                    ...
                    if not os.path.exists(path_abs): continue
                    ...
                    cursor.execute("INSERT INTO PlaylistEntity (listId, trackId, databaseUuid, nextEntityId, membershipReference) VALUES (?, ?, ?, 0, 0)", (playlist_id, track_id, db_uuid))
            ...
            self.update_status(f"Importação concluída: {processed_count} faixas em {len(tracks_by_drive)} discos.", "green")
            messagebox.showinfo("Sucesso", f"A playlist '{playlist_name}' foi distribuída e sincronizada com sucesso nos discos detectados!")
            self.destroy()

        except Exception as e:
            self.update_status(f"Erro: {e}", "red")
            messagebox.showerror("Erro de Importação", f"Não foi possível processar a playlist:\n{e}")